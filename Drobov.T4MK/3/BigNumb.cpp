...
{																						//Возвращает целую часть от деления, а также остаток в переменной remainder.
	...
	{
		int qGuessInterMax = 256;
		int qGuessInterMin = 0;
		int qGuessCenter = qGuessInterMax;

		//Осуществляем подбор числа qGuessCenter бинарным поиском.
		while (qGuessInterMax - qGuessInterMin > 1)
		{
			qGuessCenter = (qGuessInterMax + qGuessInterMin) / 2;
			...
			if (tmp > remainder)
				qGuessInterMax = qGuessCenter;
			else
				qGuessInterMin = qGuessCenter;
		}
		BigNumb tmp = divider * qGuessInterMin;
		tmp.Shift(i - 1); 			//Умножение на 256 ^ (i - 1).
		remainder = remainder - tmp;
		Result[i - 1] = qGuessInterMin;
	}
...
BigNumb BigNumb::operator+(const int& right) const
{
	return Add_Sub(*this, (BigNumb) right);
}

BigNumb BigNumb::operator-(const int& right) const
{
	return *this + (BigNumb) (-right);
}

BigNumb BigNumb::operator*(const int& right) const
{
	return Multipl(*this, (BigNumb)  right);
}

BigNumb BigNumb::operator/(const int& right) const
{
	BigNumb rem;
	return Divid(*this, (BigNumb)  right, rem);
}

BigNumb BigNumb::operator%(const int& right) const
{
	BigNumb rem;
	Divid(*this, (BigNumb)  right, rem);
	return rem;
}

//Для Python.

bool BigNumb::operator>(const int& B)
{
	return this->Compare( (BigNumb) B) > 0;
}
...
char* BigNumb::__repr__()
{
	return TakeString();
}

char* BigNumb::__str__()
{
	return TakeString();
}